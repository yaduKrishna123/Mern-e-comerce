...
import './button.css'
import { addtocart } from '../../api/allapi'

function Btn({category,name,image,price,stock}) {


  const Addtocart=async(category,name,image,price,stock,setalldata)=>{
    try{
          
      const Cartresponse=await addtocart(category,name,image,price,stock)
    if(Cartresponse.status==200){
      console.log(Cartresponse);
      alert("item added to cart")
      setalldata(Cartresponse.data)
  ...
  return (
    <div>
        <button class="button" onClick={()=>Addtocart(category,name,image,price,stock)} >
  <span>Add to car <i class="fa-solid fa-cart-shopping"></i>t</span>
